...
\section{Transmisión Macrofiscal y Gestión de Pasivos (SVAR y CIR)}
...
Por su parte, la calibración del proceso estocástico continuo de Cox-Ingersoll-Ross (CIR) para el EMBI+ (Sección \ref{sec:cir_resultados}) revela una vida media de reversión de $1.62$ años y una marcada heterocedasticidad condicional en los niveles de estrés. Esto implica que las ventanas de compresión de spreads soberanos en Argentina constituyen fenómenos transitorios: una estrategia óptima de administración de pasivos públicos debe aprovechar activamente estos períodos de compresión para alargar el perfil de vencimientos y sustituir deuda denominada en moneda extranjera por instrumentos en moneda doméstica indexados a inflación, reduciendo la exposición al riesgo de refinanciación extrema ante la eventual reversión del ciclo financiero global.
...
\subsection{Simulación Estocástica: Gráfico de Abanico de Monte Carlo (Especificación Anterior)}

Como se detalla en el Capítulo \ref{ch:resultados} (Sección \ref{sec:dsa_resultados}), la especificación de referencia de esta tesis para el componente estocástico del DSA deriva ahora su matriz de covarianza de los residuos estimados del propio SVAR restringido sobre la ventana ampliada de treinta años, y arroja una probabilidad de insolvencia hacia 2035 del $26.1\%$. Esta subsección y las Secciones \ref{sec:garch_robustez} y \ref{sec:dcc_garch} documentan, en cambio, la especificación anterior de ese componente estocástico, calibrada a mano sobre órdenes de magnitud de la volatilidad histórica argentina. Se conserva aquí en su totalidad, sin fabricar variantes de sensibilidad nuevas alrededor de la cifra del $26.1\%$ que no fueron efectivamente corridas, porque el director de tesis debe todavía evaluar cuál de las dos especificaciones integrar a la versión final del documento, en el mismo sentido en que el Capítulo \ref{ch:resultados} reencuadra ahora la calibración CIR y el modelo DCC-GARCH del riesgo soberano como especificación alternativa (Sección \ref{sec:cir_resultados}).
...
\subsection{Robustez de la calibración: desvíos estándar estimados vía GARCH (Especificación Anterior)}
\label{sec:garch_robustez}

La matriz de covarianzas utilizada en la simulación estocástica se calibró, como se detalló en el Capítulo \ref{ch:metodologia}, a partir de los desvíos estándar históricos muestrales de cada perturbación. Como chequeo de robustez, se reestimaron los desvíos estándar del resultado primario, del crecimiento del producto y de la variación del tipo de cambio real mediante un modelo GARCH(1,1) univariado por serie, mientras que la matriz de correlación histórica y los desvíos de las tasas de interés se mantuvieron en su valor calibrado original.

La Tabla \ref{tab:garch_desvios} compara ambos conjuntos de desvíos estándar anualizados.
...
Esta subsección documenta, sin alterar sus valores porque provienen de una corrida real ya efectuada, una extensión de la especificación de calibración a mano del DSA. El Capítulo \ref{ch:resultados} (Sección \ref{sec:cir_resultados}) reencuadra hoy esta extensión, junto con la calibración CIR del EMBI+, como especificación alternativa del componente estocástico del DSA, pendiente de que el director de tesis defina cuál integrar a la versión final, frente a la especificación de referencia basada en la covarianza de residuos del SVAR restringido ($26.1\%$, Sección \ref{sec:dsa_resultados} del Capítulo \ref{ch:resultados}).
...
El contraste entre los tres escenarios de la Tabla \ref{tab:probabilidad_escenarios}, que va desde una probabilidad de insolvencia prácticamente nula hasta una trayectoria de insostenibilidad segura, constituye el hallazgo sustantivo central de esta especificación anterior del DSA, y se sostiene igualmente bajo la especificación de referencia del Capítulo \ref{ch:resultados} ($26.1\%$ en el escenario de Referencia, Sección \ref{sec:dsa_resultados}): la sostenibilidad de la deuda pública argentina hacia 2035 depende, en un grado mucho mayor que de cualquier ajuste fiscal discrecional marginal, de la evolución de las variables macroeconómicas fundamentales sobre las cuales la política fiscal \textit{per se} ejerce un control solo parcial.
