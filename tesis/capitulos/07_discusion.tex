...
La descomposición de varianza del SVAR Restringido (Sección \ref{sec:svar_resultados} del Capítulo \ref{ch:resultados}) ordena las fuentes de variabilidad de la ratio de deuda a horizontes de mediano plazo: los shocks al resultado primario explican el $47.5\%$ de la varianza del error de pronóstico a 20 trimestres y los shocks a la brecha del producto un $28.6\%$ adicional, frente a una contribución conjunta de apenas el $11.5\%$ de los shocks al tipo de cambio real y al riesgo soberano, y un $12.4\%$ de la propia inercia de la deuda. El resultado es coherente con el resto del capítulo: son los desbalances primarios ---y no una regla de reacción fiscal estabilizadora, cuya ausencia documentan DOLS y el VECM--- el principal impulsor proximal de la dinámica de la deuda dentro de la muestra, seguidos por el ciclo real. Esto no contradice la evidencia sobre los canales de Ajuste Stock-Flujo (Sección \ref{sec:hoja_balance} del Capítulo \ref{ch:descriptivo}): una cosa es qué perturbaciones mueven la deuda período a período ---shocks al resultado primario y al producto--- y otra distinta es a través de qué canales se resolvieron los episodios de sobreendeudamiento ---depreciación real, licuación inflacionaria y reestructuraciones con quita---, punto que se retoma en las implicancias de economía política. Para el debate de política, la lectura es que la consolidación fiscal primaria es condición necesaria pero opera sobre una dinámica en la que el ciclo real y la volatilidad financiera siguen pesando de forma no despreciable.
