...
\midrule
(iii) Calibración (no estimación) de 2 de 5 shocks DSA ($r_d$, $r_f$) & Superable & Los otros tres shocks (resultado primario, crecimiento, tipo de cambio) usan la covarianza estimada del SVAR restringido (Sección \ref{sec:dsa_resultados} del Capítulo \ref{ch:resultados}); $r_d$ y $r_f$ siguen calibrados, al no tener serie propia en el sistema de cointegración \\
\midrule
...
2.4 & \url{fase4_variables_instrumentales.py} & IV-2SLS (VIX + BOVESPA\_VOL, ventana única), F de primera etapa, Wu-Hausman, Sargan & Tabla \ref{tab:iv2sls} \\
...
2.7 & \url{fase7_diagnosticos_robustez.py} & ACF/PACF, ARCH-LM, CUSUM/CUSUMSQ, Engle-Granger, sensibilidad temporal, probabilidades por escenario (calibración a mano), causalidad de Granger, filtro de Hamilton y covarianza del DSA vía GARCH(1,1) & Figuras \ref{fig:acf_pacf}, \ref{fig:cusum}; Tablas \ref{tab:sensibilidad_temporal}, \ref{tab:engle_granger}; probabilidades por escenario y covarianza GARCH no incorporadas a la versión final (documentadas aparte) \\
2.8 & \url{fase8_deuda_consolidada.py} & Consolida deuda SPNF + pasivos BCRA, Mejora Dim. IV & Figura \ref{fig:deuda_consolidada} (Sección \ref{sec:deuda_consolidada}) \\
...
2.10 & \url{dcc_garch.py} \newline \url{fase10_dcc_garch.py} & DCC-GARCH, correlación condicional dinámica & No incorporado a la versión final del documento (documentado aparte) \\
...
2.12 & \url{fase12_diagnosticos_complementarios.py} & VIF y número de condición; selección del orden de rezagos del VAR (AIC/BIC/HQ/FPE) y sensibilidad del rango de Johansen, superadas como tablas propias por la Etapa 2.20 (ventana única) & Tabla \ref{tab:vif} \\
...
2.14 & \url{fase14_dsa_tail_dependence.py} & Robustez del DSA a la estructura de dependencia en colas (marginales $t$ independientes, cópula Gaussiana), sobre la calibración a mano & No incorporada a la versión final del documento (documentada aparte) \\
