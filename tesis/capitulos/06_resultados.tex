...
Este capítulo contrasta empíricamente las hipótesis planteadas en el Capítulo \ref{ch:introduccion} mediante econometría de series de tiempo. El paso desde el análisis descriptivo del Capítulo \ref{ch:descriptivo} hacia la estimación econométrica permite evaluar los determinantes de la Función de Reacción Fiscal argentina, sobre la ventana original 2004--2025 ($n=88$, Secciones \ref{sec:dfgls}--\ref{sec:sensibilidad_temporal}) y sobre la ventana ampliada 1996--2025 ($n=120$, treinta años exactos, Secciones \ref{sec:resultados_vecm}--\ref{sec:tvecm_resultados}), esta última mediante el Modelo de Vectores con Corrección de Error (VECM), el SVAR restringido y el Threshold VECM, que reemplazan a DOLS como técnica de referencia a pedido del director de tesis (Secciones \ref{sec:vecm} y \ref{sec:svar_metodologia} del Capítulo \ref{ch:metodologia}). La evidencia recogida es mayormente débil o estadísticamente no concluyente: la regla de reacción fiscal lineal de Bohn no opera de forma estable en ninguna de las dos ventanas ni bajo ninguna de las técnicas aplicadas. Dicha inoperancia estadística constituye el hallazgo central del capítulo, resulta coherente con la hipótesis de fatiga fiscal que motiva la tesis y se analiza en profundidad en el Capítulo \ref{ch:discusion}. La descomposición de varianza del SVAR restringido, en cambio, sí cambia de manera sustantiva al extender la ventana a treinta años (Sección \ref{sec:svar_resultados}), y esa reversión se discute con el mismo criterio de transparencia que el resto del capítulo.
...
La especificación de referencia del Análisis de Sostenibilidad de la Deuda estocástico (Sección \ref{sec:dsa_metodologia} del Capítulo \ref{ch:metodologia}) sustituye el proceso CIR y el modelo DCC-GARCH de una especificación anterior (documentada como alternativa en la Sección \ref{sec:cir_resultados} siguiente) por una matriz de correlación derivada directamente de la covarianza de residuos estructurales del SVAR restringido de la Sección \ref{sec:svar_resultados}, estimado sobre la ventana ampliada de treinta años. Bajo esta especificación, la probabilidad de que la deuda consolidada supere el 100\% del PIB hacia 2035 es $\mathbf{26{,}1\%}$, con una mediana de $82{,}8\%$ del PIB en el escenario de Referencia determinista. Es un resultado del mismo orden de magnitud que la especificación anterior ($31{,}2\%$ con la matriz calibrada a mano, $29{,}8\%$ con CIR, $32{,}5\%$ con DCC-GARCH), pero no idéntico: la matriz de correlación fundada en el SVAR estimado, a diferencia de la calibrada a mano, incorpora directamente el hallazgo de la Sección \ref{sec:svar_resultados} de que el riesgo soberano y el tipo de cambio real están, sobre esta ventana, menos correlacionados entre sí de lo que asumía la calibración original, lo que angosta moderadamente la cola derecha de la distribución simulada.

\section{Especificación Alternativa: Calibración del Proceso Estocástico CIR del EMBI+}
\label{sec:cir_resultados}

Como especificación alternativa a la de la sección anterior, documentada en su totalidad para que el director de tesis evalúe cuál integrar a la versión final del documento, la Tabla \ref{tab:cir_calibracion} resume la calibración por Máxima Verosimilitud exacta del proceso de Cox-Ingersoll-Ross (CIR 1985) para la prima de riesgo soberano.
...
Al integrar las 1.000 trayectorias continuas del proceso CIR en el módulo de proyección estocástica del DSA hacia 2035 (sustituyendo la tasa de refinanciación externa fija por $r_{f,t} = r_{US,t} + \text{CIR}_t$), la probabilidad estimada de insolvencia ($P(d_{2035} > 100\%)$) se sitúa en $\mathbf{29.8\%}$, convergiendo con gran consistencia con la cifra central de la especificación calibrada a mano que sostenía esta sección ($31.2\%$) y sus robusteces ($30.4\%$ a $31.9\%$), y del mismo orden de magnitud que la especificación de referencia actual, fundada en el SVAR ($26.1\%$, Sección \ref{sec:dsa_resultados}).
...
Cinco estrategias de estimación, sobre dos ventanas muestrales, convergen en un diagnóstico predominantemente común sobre la reacción fiscal, con una divergencia sustantiva en la descomposición de varianza del canal de transmisión, que exige calificación explícita. Sobre la ventana original (2004--2025), ahora con series reales de EMBI+ y TCRM: el contraste de Johansen indica, sin ambigüedad, exactamente una relación de cointegración ($r=1$) entre las cuatro variables del sistema; DOLS no encuentra una reacción lineal significativa al endeudamiento ($\rho=-0.0071$, $p=0.564$); el VECM sobre ese mismo sistema, ya sin necesidad de imponer el rango por motivo teórico, replica el resultado nulo ($\alpha_{pb}=0.0014$, $p=0.559$); el umbral de Hansen sobre $\Delta pb_t$ rechaza la linealidad en torno a $\tau^* \approx 2083$ pb ($p<0.001$).

Sobre la ventana ampliada de treinta años (1996--2025, $n=120$), que reemplaza a la de 108 observaciones como técnica de referencia: el contraste de Johansen no rechaza la ausencia de cointegración para ningún inicio de muestra entre 1996T1 y 1999T1 (Sección \ref{sec:resultados_vecm}), un resultado más contundente que el de la ventana de 108 observaciones; el VECM replica, con mayor claridad aún, el resultado nulo de la reacción fiscal ($\alpha_{pb}\approx 0$, $p=0.996$) y encuentra además un canal de ajuste vía riesgo soberano significativo ($\alpha_{EMBI}=6.356$, $p=0.025$) que la ventana de 108 observaciones no alcanzaba a identificar; el TVECM multivariado localiza un umbral prácticamente idéntico al de Hansen en $\Delta pb_t$ ($\tau^*=2104$ pb) y replica la inversión de signo en la velocidad de ajuste fiscal entre regímenes ($\alpha_{pb}$ pasa de $+0.001$ a $-0.007$), sin alcanzar tampoco aquí significatividad en el contraste Sup-LM ($p=0.632$); y la descomposición de varianza del SVAR restringido se invierte respecto de la ventana original: la inercia propia de la deuda pasa de explicar el $12.4\%$ al $54.2\%$ de su propia varianza a 20 trimestres, y el canal cambiario-riesgo conjunto ($25.8\%$) supera ahora al fiscal-real conjunto ($20.0\%$), verificado como no atribuible a la única aproximación de datos que introduce esta ventana (Sección \ref{sec:svar_resultados}). El DSA estocástico, con una covarianza derivada de esta misma estimación del SVAR en lugar del proceso CIR/DCC-GARCH de una especificación anterior (documentada aparte más abajo), sitúa la probabilidad de insolvencia a 2035 en $26.1\%$.

En síntesis: la evidencia de una reacción fiscal lineal, estable y de nivel sigue siendo predominantemente débil o nula, y se fortalece con la ventana de treinta años en lugar de debilitarse (el veredicto que sostienen DOLS, el VECM en las tres ventanas y, ahora con más precisión, el propio VECM ampliado); la evidencia de un umbral de fatiga fiscal se mantiene igual de mixta que antes bajo las especificaciones no lineales (significativa en Hansen sobre $\Delta pb_t$ y en su umbral gemelo del TVECM, no significativa en el contraste Sup-LM de este último); y la lectura sobre el canal dominante de transmisión hacia la deuda cambia de forma sustantiva con la ventana más larga, de un canal fiscal-real a uno de inercia propia y riesgo-cambiario, una reversión que el Capítulo \ref{ch:discusion} discute en profundidad antes de evaluar sus implicancias teóricas sobre la solvencia intertemporal.
