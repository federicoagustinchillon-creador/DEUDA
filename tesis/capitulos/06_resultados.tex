...
A diferencia de la ventana ampliada de 108 observaciones, donde excluir un único año revertía el diagnóstico, en la ventana de treinta años el resultado es más contundente: ningún punto de inicio entre 1996T1 y 1999T1, cuatro alternativas distintas, rechaza la ausencia de cointegración; recién al excluir también 1998 y 1999 completos (inicio en 2000T1) el estadístico de Traza supera al crítico. Esta fragilidad es coherente con la literatura sobre la distorsión del contraste de Johansen ante quiebres estructurales severos no modelados \citep{gregory1996,johansen2000}: treinta años de muestra continua no ofrecen, por sí solos, una imagen invariante de la cointegración de largo plazo, sino una mayor sensibilidad derivada de abarcar regímenes macroeconómicos disímiles. Se impone en consecuencia un rango de cointegración $r=1$ por motivo teórico, el mismo criterio ya aplicado en la Sección \ref{sec:johansen_cap6}, documentando explícitamente que la imposición no es una elección neutral.
...
Asimismo, las variables del sistema ($pb_t$, $d_t$, $\tilde{y}_t$ y $risk_t$) se especifican en niveles (o en desviación porcentual de su tendencia en el caso de $\tilde{y}_t$). Dado que el Resultado Primario ($pb_t$) y la Brecha del Producto ($\tilde{y}_t$) toman valores negativos durante episodios de déficit y contracción económica, la transformación logarítmica resulta matemáticamente inviable. Conservar la escala en niveles preserva una interpretación directa y homogénea de los coeficientes como puntos porcentuales del PIB por unidad de variación de los regresores.
