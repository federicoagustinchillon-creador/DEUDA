...
\midrule
\multirow{2}{*}{1 Trimestre} & Deuda Pública ($d_t$) & 1.24 & 4.85 & 8.92 & 14.30 & 70.69 \\
 & Riesgo País ($\text{EMBI}_t$) & 5.12 & 2.45 & 84.15 & 7.82 & 0.46 \\
\midrule
\multirow{2}{*}{4 Trimestres} & Deuda Pública ($d_t$) & 3.45 & 6.20 & 16.80 & 28.50 & 45.05 \\
 & Riesgo País ($\text{EMBI}_t$) & 8.30 & 4.10 & 68.25 & 18.15 & 1.20 \\
\midrule
\multirow{2}{*}{8 Trimestres} & Deuda Pública ($d_t$) & 5.10 & 7.85 & 22.40 & 34.60 & 30.05 \\
 & Riesgo País ($\text{EMBI}_t$) & 10.45 & 5.20 & 58.60 & 24.30 & 1.45 \\
\midrule
\multirow{2}{*}{20 Trimestres} & Deuda Pública ($d_t$) & 6.80 & 8.15 & 26.50 & 38.90 & 19.65 \\
 & Riesgo País ($\text{EMBI}_t$) & 12.10 & 5.80 & 52.40 & 27.90 & 1.80 \\
\bottomrule
\end{tabular}%
}
\begin{flushleft}
\footnotesize \textit{Nota.} Identificación estructural por restricciones macroeconómicas de Blanchard-Perotti ($\alpha_y=0.95$, $a_{23}=a_{24}=0$) e identidad de acumulación de Favero-Giavazzi. Elaboración propia sobre la muestra 2004--2025.
\end{flushleft}
\end{table}
...
Como se desprende de la Figura \ref{fig:svar_irf} y la Tabla \ref{tab:svar_fevd}, los shocks cambiarios reales ($\text{TCRM}$) y de riesgo soberano ($\text{EMBI}$) explican conjuntamente más del $65\%$ de la varianza del error de pronóstico de la deuda a horizontes de 5 años ($38.9\%$ y $26.5\%$, respectivamente). En contraste, el shock de superávit primario propio apenas explica un $8.15\%$ de la variabilidad de la deuda. Este resultado empírico formaliza la dominancia del canal de valoración cambiaria y corte de crédito externo por sobre la disciplina fiscal discrecional, en plena concordancia con el Paradigma de la Restricción Externa.
...
Observaciones por Régimen & 74 trimestres ($84.1\%$) & 14 trimestres ($15.9\%$) \\
Velocidad de Ajuste Fiscal ($\alpha_{pb}$) & $\mathbf{+0.0064}$ & $\mathbf{-0.0079}$ \\
Estadístico Sup-LM de Linealidad & \multicolumn{2}{c}{$\text{Sup-LM} = 52.24 \quad (p\text{-valor bootstrap} = 0.061)$} \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Nota.} Vector de cointegración estimado por Johansen. El umbral estimado por búsqueda de grilla bidimensional ($2080.68$ pb) coincide estrechamente con el umbral de Hansen en diferencias ($2083$ pb). Elaboración propia.
\end{flushleft}
\end{table}

El TVECM confirma que en el Régimen 1 ($\text{EMBI} \le 2080$ pb), la velocidad de ajuste del superávit es positiva ($\alpha_{pb}^{(1)} = +0.0064$), operando como un estabilizador débil pero corrector. Sin embargo, cuando el riesgo cruza el umbral crítico ($\text{EMBI} > 2080$ pb), la velocidad de ajuste colapsa y cambia de signo ($\alpha_{pb}^{(2)} = -0.0079$), reflejando la inviabilidad política y macroeconómica de generar superávits compensatorios en plena crisis de financiamiento.
...
\midrule
Velocidad de Reversión ($\kappa$) & 0.4280 & 0.3850 \\
Nivel de Equilibrio ($\theta$, en pb) & 650.00 & 720.00 \\
Volatilidad de Difusión ($\sigma$) & 18.4200 & 19.8500 \\
Vida Media de Reversión ($t_{1/2} = \ln(2)/\kappa$) & 1.62 años & 1.80 años \\
Condición de Feller ($2\kappa\theta > \sigma^2$) & $556.40 > 339.30 \implies \mathbf{Cumple}$ & $554.40 > 394.02 \implies \mathbf{Cumple}$ \\
Ratio de Feller ($2\kappa\theta / \sigma^2$) & $\mathbf{1.6402 > 1.0}$ & $\mathbf{1.4070 > 1.0}$ \\
Criterio AIC (CIR vs. AR(1)) & $\mathbf{1142.3} < 1188.6 \implies \text{Prefiere CIR}$ & $\mathbf{1410.2} < 1465.8 \implies \text{Prefiere CIR}$ \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Nota.} Estimación por MLE exacta sobre la función de densidad Chi-cuadrado no central. La condición de Feller garantiza la inaccesibilidad de la frontera en cero. Elaboración propia.
\end{flushleft}
\end{table}
